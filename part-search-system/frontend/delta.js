...
// 安全转义: HTML 文本 (防止存储型 XSS)
function escapeHtml(v) {
  if (v === null || v === undefined) return '';
  return String(v).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;').replace(/'/g, '&#39;');
}
// 安全转义: 内联 onclick 中的 JS 字符串参数
function jsStr(v) {
...
function loadDelta(page) {
  ...
  fetch(url)
    ...
    .catch(function(err) {
      deltaState.loading = false;
      document.getElementById('deltaList').innerHTML =
        '<p style="color:var(--danger);padding:20px;">加载失败: ' + escapeHtml(err) + '</p>';
    });
}
  ...
  var html = deltas.map(function(d) {
    ...
    var changeTags = changedFields.map(function(c) {
      ...
      return '<span class="change-tag ' + c.change_type +
             ...
             escapeHtml(c.business) + ': ' + changeTypeLabel(c.change_type) + '</span>';
    }).join('');
    ...
    var fieldRows = d.changes.map(function(c) {
      return '<div class="delta-field-row">' +
             '<div class="field-label">' + escapeHtml(c.business) + '</div>' +
             '<div class="old-val ' + (c.old_value ? '' : 'empty') + '">' + escapeHtml(c.old_value || '—') + '</div>' +
             '<div class="arrow ' + c.change_type + '">' + changeTypeLabel(c.change_type) + '</div>' +
             '<div class="new-val ' + (c.new_value ? '' : 'empty') + '">' + escapeHtml(c.new_value || '—') + '</div>' +
             '</div>';
    }).join('');
    ...
    return '<div class="delta-item" onclick="toggleDeltaExpand(this)">' +
           ...
           'onclick="event.stopPropagation();openDeltaDetail(\'' + jsStr(d.part_number) + '\')" ' +
           'title="点击查看详情">' + escapeHtml(d.part_number) + '</span>' +
           '<span class="match-badge" style="background:' + matchColor + '22;color:' + matchColor +
           ';border:1px solid ' + matchColor + '55;">' + escapeHtml(matchLabel) + '</span>' +
           '</div>' +
...
function openDeltaDetail(partNumber) {
  ...
  modal.innerHTML =
    ...
    '<span style="color:#ff6b6b;">' + escapeHtml(fromStage) + '</span>' +
    ' <span style="margin:0 6px;">→</span> ' +
    '<span style="color:#00ff88;">' + escapeHtml(toStage) + '</span>' +
    ' · PN: <strong style="color:var(--silver);">' + escapeHtml(partNumber) + '</strong>' +
    '</div>' +
    '</div>' +
  ...
  fetch('/api/delta_detail?pn=' + encodeURIComponent(partNumber) +
    ...
    .then(function(res) {
      if (!res.success) {
        document.getElementById('deltaDetailContent').innerHTML =
          '<p style="color:var(--danger);text-align:center;">' + escapeHtml(res.error || '查询失败') + '</p>';
        return;
      }
    ...
    .catch(function(err) {
      document.getElementById('deltaDetailContent').innerHTML =
        '<p style="color:var(--danger);text-align:center;">加载失败: ' + escapeHtml(err) + '</p>';
    });
}
...
function renderDeltaDetailContent(data, fromStage, toStage) {
  ...
  html += '<div style="display:flex;align-items:center;justify-content:space-between;' +
    ...
    (comp.from_exists ? '' : (escapeHtml(fromStage) + ' 无数据 · ')) +
    (comp.to_exists ? '' : (escapeHtml(toStage) + ' 无数据')) +
    '</div>' +
    '</div>';
...
function renderCompareTable(fields, fromStage, toStage) {
  ...
  var rows = fields.map(function(f) {
    ...
    return '<tr style="background:' + rowBg + ';">' +
      ...
      'color:var(--text-secondary);width:180px;">' + escapeHtml(f.business) + '</td>' +
      ...
      '">' + (f.from_value ? escapeHtml(f.from_value) : '<span style="color:var(--text-tertiary);">—</span>') + '</td>' +
      ...
      '">' + (f.to_value ? escapeHtml(f.to_value) : '<span style="color:var(--text-tertiary);">—</span>') + '</td>' +
      '</tr>';
  }).join('');
